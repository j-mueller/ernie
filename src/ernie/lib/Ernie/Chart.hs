...
import Control.Lens (at, makePrisms, (%=), (.=), _1, _Just)
...
task ::
    ...
    -- | Estimated duration
    (TaskDuration m) ->
    -- | List of tasks that the new task depends on
    [TaskID] ->
...
type PERTChart = DependencyGraph TaskID (Task (Maybe (PERTEstimate Days)))
